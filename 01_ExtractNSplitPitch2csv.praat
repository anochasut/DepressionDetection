...
for i from 1 to n
    ...
    # Prepare out fname
    outfname$ = replace$ (file_name$, "_AUDIO.wav", "_AUDIO.pitch", 0)
    outfname$ = folder$ + outfname$

    pitch = To Pitch (filtered autocorrelation): 0.5, 50, 800, 3, "no", 0.03, 0.09, 0.5, 0.055, 0.35, 0.14
    Save as short text file: outfname$
    ...
    #    #Select outer viewport: -0.5, 1, -0.5, 3.5
    #    pitch = To Pitch (filtered autocorrelation): 0.5, 50, 800, 3, "no", 0.03, 0.09, 0.5, 0.055, 0.35, 0.14
    #    Save as short text file: outfname$
